  ...
  const validationSchema = Yup.object({
    degree: Yup.string()
      ...
      .matches(/^[a-zA-Z\s]+$/, 'Degree must only contain alphabets and spaces')
      .test('degree-not-empty', 'Please select or enter a degree', (value) => {
        if (value === 'Other' && !value.trim()) {
          return false;
        }
        return !!value;
    ...
    .matches(/^[a-zA-Z\s]*$/, 'Branch must only contain alphabets and spaces'),
      ...
      <Form.Group className="mb-3">
        ...
        {selectedDegree === 'Other' && (
          <Form.Control
            ...
            onChange={(e) => {
              const value = e.target.value;
              // Ensure that only alphabets and spaces are entered
  ...
  <Field name="branch">
    {({ field }) => (
      <Form.Group className="mb-3">
        <Form.Label style={{ color: "white" }}>Branch *</Form.Label>
        {selectedDegree === "Other" ? (
          // Render input field for branch if "Other" degree is selected
          <Form.Control
            type="text"
            ...
            onChange={(e) => {
              const value = e.target.value;
              // Restrict special characters, only allow alphabets and spaces
              if (/^[a-zA-Z\s]*$/.test(value)) {
  ...
  //     "Cyber Security", "Cloud Computing", "Artificial Intelligence", "Data Science", 
  //     "System Management", "Full Stack Development", "Data Analytics", "Software Development", "Mobile Application Development"
  //   ],
  //   "Other": ["N/A"]
  // };
